...
class SQHelper():
    ...
    def __check_database_is_initialised(self):
        if self.con is None:
            ...
            print("Trying to operate on a database which doesn't exist " "or hasn't been initialised.")
            return False
        return True
    ...
    def insert(self, table, values):
        ...
        if self.__check_database_is_initialised() is False:
            print("__check_database_is_initialised is False")
            return False
        if type(values) is list:
            ...
            if list(sorted(values.keys())) != self.get_column_names(table):
                ...
                print("get_column_names",self.get_column_names(table))
                print("values_keys",list(sorted(values.keys())))
                return False
            ...
            print("Trying to insert data into table {} "
                      "which is not of type list. Passed type {}".format(table, type(values)))
            return False
        ...
        try:
            ...
            print("Exception: {} when inserting data into table {}. "
                      "Possible data length mismatch, invalid table".format(e, table))
            return False
        self.con.commit()
        return True
